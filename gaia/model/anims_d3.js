...
class Graph {
    ...
    initializeGraph() {
        for (const boneName in this.bones) {
            const data = this.bones[boneName];
            this.nodes[boneName] = {
                position: Array.isArray(data.pos) ? [...data.pos] : [data.pos.x || 0, data.pos.y || 0, data.pos.z || 0],
                rotation: new THREE.Quaternion(0, 0, 0, 1),
                mass: 1,
                velocity: [0, 0, 0],
                forces: [0, 0, 0],
        ...
        this.muscles.forEach(muscle => {
            this.edges.push({
                source: muscle.origin,
                target: muscle.insertion,
                attachPoints: muscle.attachPoints || [[0,0,0], [0,0,0]],
                forceAmplitude: 0,
                maxForce: 1 * ((muscle.geomRelaxed?.length || 1) - (muscle.geomContracted?.length || 0.8)),
...
export function createAnimations(boneMap, muscles) {
    ...
    for (const nodeName in boneMap) {
        ...
        bones[nodeName] = {
            pos: bone.position ? [bone.position.x, bone.position.y, bone.position.z] : [0, 0, 0],
